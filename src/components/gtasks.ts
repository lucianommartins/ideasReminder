...
const DEDICATED_TASK_LIST_NAME = 'GDM DevRel tasks';
...
async function ensureDedicatedTaskListExists(tasksService: tasks_v1.Tasks): Promise<string> {
    ...
    console.log(`gtasks.ts: Dedicated task list not found. Creating it.`);
...
export async function getTasksInList(senderId: string): Promise<tasks_v1.Schema$Task[] | string> {
    try {
        ...
        const tasklistId = await findDedicatedTaskListId(tasksService);
        if (!tasklistId) {
            // If the list doesn't exist, there are no tasks.
            return [];
        }
...
export async function getFormattedTasksString(senderId: string): Promise<string> {
    ...
    let hasDueDate = false; // Flag to check if any task has a due date

    const tasksString = tasks
        .map((task, index) => {
            const title = `*${index + 1}. ${task.title || 'Untitled Task'}*`;
            
            // Indent the entire notes block for readability.
            let notes = '';
            if (task.notes) {
                // Prepend each part of the notes with an indent.
                notes = `\n  ${task.notes.replace(/\n\n/g, '\n  ')}`;
            }
            ...
            if (task.due) {
                hasDueDate = true;
                // The 'due' field is an RFC 3339 timestamp, so we extract the date part.
...
export async function deleteGoogleTask(senderId: string, taskTitle: string): Promise<string> {
    try {
        ...
        // We need to list tasks from that specific list to find the one to delete.
        const tasksResult = await tasksService.tasks.list({
            tasklist: tasklistId,
            showCompleted: false,
            maxResults: 100 // Assuming user won't have more than 100 tasks to search through
